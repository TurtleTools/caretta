...
def write_superposed_pdbs(
        input_dir, cleaned_pdb_folder, alignment, output_pdb_folder, verbose: bool = False
):
    """
    ...
    if len(core_indices) < len(alignment[reference_name]) // 2:
        ...
        write_superposed_pdbs_references(input_dir, cleaned_pdb_folder, alignment, output_pdb_folder, verbose=verbose)
    else:
        write_superposed_pdbs_core(input_dir, cleaned_pdb_folder, alignment, reference_name, output_pdb_folder)


def write_superposed_pdbs_core(input_dir, cleaned_pdb_folder, alignment, reference_name, output_pdb_folder):
    """
    ...
    for name in alignment:
        ...
        if input_dir:
            pdb = pd.parsePDB(
                str(input_dir / f"{name}")
            )
        pdb = pd.applyTransformation(transformation, pdb)
        pd.writePDB(str(output_pdb_folder / f"{name}"), pdb)


def write_superposed_pdbs_reference(input_dir, cleaned_pdb_folder, alignment, reference_name, output_pdb_folder, verbose=True):
    """
